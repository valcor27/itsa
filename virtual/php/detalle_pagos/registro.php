<?php
    ...
    switch($proceso){
        ...
        break;
        case 'Edicion':
            /**Corroboramos que no sea mas grande la suma de los montos de los detalles a la del monto total del pago excluyendo el monto del detalle a editar porque se va a editar*/
            $consulta_comprobacion = "SELECT IFNULL(SUM(monto_detalle_pago), 0) AS total_detalle_monto FROM detalle_pagos WHERE pagos_id_pago = '$id_pago_padre' AND id_detalle_pago != '$id' AND estatus = '1'";
            $resultado_comprobacion = mysqli_query($conexion_database, $consulta_comprobacion);
            // Validamos si se obtuvo un resultado
            if($row_comprobacion = mysqli_fetch_array($resultado_comprobacion)){
                $total_monto_detalle_base = $row_comprobacion["total_detalle_monto"];
