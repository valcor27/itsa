<?php
    ...
    //$dato = null;//si no tiene ningun valor lo pasamos a nulo
    ...
    if($ideliminar > 0){
        $consulta_eliminar="DELETE FROM detalle_pagos WHERE id_detalle_pago='$ideliminar' LIMIT 1";
        $resultado_elimina=mysqli_query($conexion_database, $consulta_eliminar);
    }
    /*--------------------------------------------------------------------------------------------------------------*/
    ...
    //if(empty($dato)){
        /**Si viene vacio nos mostrara todos los registros que correspondan al pago que seleccionamos y tengan estatus de 1 */
        $consulta_2 = "SELECT id_detalle_pago AS id, monto_detalle_pago, nombreUnidad FROM detalle_pagos INNER JOIN estructuraorganica ON detalle_pagos.unidad_clave_unidad = estructuraorganica.claveUnidad WHERE pagos_id_pago = '$id_pago_padre' AND detalle_pagos.estatus = '1' ORDER BY id_detalle_pago DESC";

    /*}else{
        //si no viene vacio, es decir se selecciono un departamento filtramos por el departamento el pago padre y con estatus sea 1
        $consulta_2 = "SELECT id_detalle_pago AS id, monto_detalle_pago, nombreUnidad FROM detalle_pagos INNER JOIN estructuraorganica ON detalle_pagos.unidad_clave_unidad = estructuraorganica.claveUnidad WHERE unidad_clave_unidad = '$dato' AND pagos_id_pago = '$id_pago_padre' AND detalle_pagos.estatus = '1' ORDER BY id_detalle_pago DESC";
    }*/
    ...
    if($no_filas > 0){
        ...
        while($row = mysqli_fetch_array($registro_2)){
            ...
            $tabla = $tabla.'
                                <tr>
                                    <td align="center">
                                        <button type="button" class="btn btn-warning" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Actualizar Detalle Pago" onclick="Actualizar_detalle_pago('.$id.');">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button type="button" class="btn btn-danger" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Eliminar Detalle Pago" onclick="Eliminar_detalle_pago('.$id.');">
