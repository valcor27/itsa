<?php 
  ...
  switch($proceso){
      case 'Registro':
        ...
        $consulta_comparar = "SELECT id_materia FROM materia WHERE nombre_materia = '$nombre' AND division_id_division = '$division' AND estatus = '1' LIMIT 1";
        ...
        if($filas_comparar == 0){
            ...
            $inserta = "INSERT INTO  materia(nombre_materia, division_id_division, nombre_division, estatus,
    ...
    case 'Edicion':
        ...
        $consulta_comparar = "SELECT id_materia FROM materia WHERE id_materia <> '$id' AND nombre_materia = '$nombre' AND division_id_division = '$division' AND estatus = '1' LIMIT 1";
        $resultado_comparar = mysqli_query($conexion_database, $consulta_comparar);
        $filas_comparar = mysqli_num_rows($resultado_comparar);
        ...
        /**------------------------------------------------------------------- */
        if($filas_comparar == 0){
            $actualiza = "UPDATE materia SET nombre_materia = '$nombre', division_id_division = '$division', nombre_division = '$nombre_division',
            fecha_movimiento = '$fecha', ultimo_movimiento = '$proceso', 
            usuario_movimiento = '$usuario' WHERE id_materia = '$id' LIMIT 1";
  ...
  $consulta = "SELECT id_materia AS id, nombre_materia, nombre_division FROM materia WHERE nombre_materia = '$nombre' AND division_id_division = '$division' AND estatus = '1' LIMIT 1";
  ...
  while($row = mysqli_fetch_array($registro)){
    ...
    $n_materia = $row["nombre_materia"];
    $tabla = $tabla.'
          <tr>
            <td align = "center">
              <button type="button" class="btn btn-warning" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Actualizar Materia" onclick="Actualizar_materia('.$id.');">
                <i class="fas fa-edit"></i>
              </button>
              <button type="button" class="btn btn-danger" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Eliminar Matria" onclick="Eliminar_materia('.$id.');">
