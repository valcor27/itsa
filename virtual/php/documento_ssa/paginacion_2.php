...
/**------------------------------------------------------------------------- */
/**--------------------Verificamos que solo sean los documentos que esten en el departamento del usuario---------- */
$consulta_documentos_departamento = "SELECT DISTINCT h1.documento_id_documento
FROM historial_movimientos h1
WHERE h1.departamento_actual = '$departamento_usuario'
AND NOT EXISTS (
  SELECT 1
  FROM historial_movimientos h2
  WHERE h2.documento_id_documento = h1.documento_id_documento
    AND h2.id_historial > h1.id_historial
    AND h2.departamento_actual <> '$departamento_usuario'
);


";
$resultado = mysqli_query($conexion_database, $consulta_documentos_departamento);

$documentos = array();

if ($resultado){ 
    while ($row = mysqli_fetch_assoc($resultado)) {
        $documentos[] = $row['documento_id_documento'];
    }
} 
$documentosIDs = implode(",", $documentos);
//echo $documentosIDs;
/**--------------------------------------------------------------------------------------------------------------- */

/*-----Consulta numero de datos en BD---------------------------------------------------------------------------*/
$consulta_1="SELECT id_documento FROM documentos WHERE id_documento IN ($documentosIDs) AND (folio LIKE '%$dato%' OR asunto LIKE '%$dato%')";
...
$consulta_2="SELECT id_documento as id, folio, asunto, fecha_creacion, estatus FROM documentos WHERE id_documento IN ($documentosIDs) AND (folio LIKE '%$dato%' OR asunto LIKE '%$dato%') ORDER BY id_documento ASC LIMIT $limit, $nroLotes";
...
$tabla = $tabla.'<div class="table-responsive">
    ...
    // Manejo de error o mensaje informativo si no se encontraron resultados
    $nroProductos = 0; // O cualquier otro valor que desees asignar en esta situación
    $tabla = '';
  $lista_info = '';
$lista = '';
$lista_info = ' Pag. 1 / 1 ';
...
$lista = $lista.'
    <li class="page-item active">
      <a class="page-link" href="#Paginar" onclick="Pagination_documentos();">1</a>
    </li>';
$lista = $lista.'
  <li class="page-item disabled">
    <a class="page-link" href="#Siguiente">
