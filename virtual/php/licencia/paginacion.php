<?php
    ...
    if($ideliminar > 0){
        $eliminar = "UPDATE licencia
        SET estatus = '0', fecha_movimiento = '$fecha', ultimo_movimiento = '$proceso', usuario_movimiento = '$usuario'
        WHERE id_licencia = '$ideliminar' LIMIT 1";
        $resultado_eliminar = mysqli_query($conexion_database, $eliminar);
    }
    /**----------------------------------------------------------- */
    /**Consultamos el numero de datos en BD----------------------- */
    $consulta_1 = "SELECT id_licencia FROM licencia WHERE (folio_l LIKE '%$dato%' OR expediente_l LIKE '%$dato%' OR nombre_empleado_l LIKE '%$dato%') AND estatus = '1'";
    ...
    if($paginaActual > 1){
        $lista = $lista.'
        <li class="page-item">
            <a class="page-link" href="#Anterior" aria-label="Previous" onclick="Pagination_licencia('.($paginaActual-1).');">
        ...
        if($i == $paginaActual){
            $lista = $lista.'
                <li class="page-item active">
                    <a class="page-link" href="#Paginar" onclick="Pagination_licencia('.$i.');">'.$i.'</a>
                </li>
            ';
        }else{
            $lista = $lista.'
    ...
    if($paginaActual < $nroPaginas){
        $lista = $lista.'
            <li class="page-item">
                <a class="page-link" href="#Siguiente" aria-label="Next" onclick="Pagination_licencia('.($paginaActual+1).');">
    ...
    $consulta_2 = "SELECT id_licencia AS id, expediente_l, nombre_empleado_l, folio_l, fecha_inicio_l, fecha_fin_l, concepto FROM licencia WHERE (folio_l LIKE '%$dato%' OR expediente_l LIKE '%$dato%' OR nombre_empleado_l LIKE '%$dato%') AND estatus = '1' ORDER BY id_licencia DESC LIMIT $limit, $nroLotes";
