<?php
    ...
    if($ideliminar > 0){
        $eliminar = "UPDATE incidencia
        SET estatus = '0', fecha_movimiento = '$fecha', ultimo_movimiento = '$proceso', usuario_movimiento = '$usuario'
        WHERE id_incidencia = '$ideliminar' LIMIT 1";
        $resultado_eliminar = mysqli_query($conexion_database, $eliminar);
    }
    /**----------------------------------------------------------- */
    /**Consultamos el numero de datos en BD----------------------- */
    $consulta_1 = "SELECT id_incidencia FROM incidencia WHERE (folio_incidencia LIKE '%$dato%' OR expediente LIKE '%$dato%' OR nombre_empleado LIKE '%$dato%') AND estatus = '1'";
    ...
    if($paginaActual > 1){
        $lista = $lista.'
        <li class="page-item">
            <a class="page-link" href="#Anterior" aria-label="Previous" onclick="Pagination_incidencia('.($paginaActual-1).');">
        ...
        if($i == $paginaActual){
            $lista = $lista.'
                <li class="page-item active">
                    <a class="page-link" href="#Paginar" onclick="Pagination_incidencia('.$i.');">'.$i.'</a>
                </li>
            ';
        }else{
            $lista = $lista.'
    ...
    if($paginaActual < $nroPaginas){
        $lista = $lista.'
            <li class="page-item">
                <a class="page-link" href="#Siguiente" aria-label="Next" onclick="Pagination_incidencia('.($paginaActual+1).');">
    ...
    $consulta_2 = "SELECT id_incidencia AS id, expediente, nombre_empleado, folio_incidencia, fecha_inicio, fecha_fin, id_clave_mov FROM incidencia WHERE (folio_incidencia LIKE '%$dato%' OR expediente LIKE '%$dato%' OR nombre_empleado LIKE '%$dato%') AND estatus = '1' ORDER BY id_incidencia DESC LIMIT $limit, $nroLotes";
