...
$consulta = "SELECT
		historico_documentos.folio AS folio_documento,
		...
		JOIN
			historico_documentos ON historial_movimientos.documento_id_documento = historico_documentos.id_documento
		LEFT JOIN
		...
		WHERE historial_movimientos.documento_id_documento = '$id' AND historico_documentos.anio_historico = '$anio' ORDER BY historial_movimientos.id_historial
	";
$registro = mysqli_query($conexion_database, $consulta);
$no_filas = mysqli_num_rows($registro);
/**------------------------------------------------------------ */
/**Creamos la timeline del historial de documentos------------- *
