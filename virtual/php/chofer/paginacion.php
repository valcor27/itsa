<?php
    ...
    if($ideliminar > 0){
        $eliminar = "UPDATE chofer
        SET estatus = '0', fecha_movimiento = '$fecha', ultimo_movimiento = '$proceso', usuario_movimiento = '$usuario'
        WHERE id_chofer = '$ideliminar' LIMIT 1";
        $resultado_eliminar = mysqli_query($conexion_database, $eliminar);
    }
    /**----------------------------------------------------------- */
    /**Consultamos el numero de datos en BD----------------------- */
    $consulta_1 = "SELECT id_chofer FROM chofer WHERE (expediente_chofer LIKE '%$dato%' OR nombre_chofer LIKE '%$dato%') AND estatus = '1'";
    ...
    if($paginaActual > 1){
        $lista = $lista.'
        <li class="page-item">
            <a class="page-link" href="#Anterior" aria-label="Previous" onclick="Pagination_chofer('.($paginaActual-1).');">
        ...
        if($i == $paginaActual){
            $lista = $lista.'
                <li class="page-item active">
                    <a class="page-link" href="#Paginar" onclick="Pagination_chofer('.$i.');">'.$i.'</a>
                </li>
            ';
        }else{
            $lista = $lista.'
    ...
    if($paginaActual < $nroPaginas){
        $lista = $lista.'
            <li class="page-item">
                <a class="page-link" href="#Siguiente" aria-label="Next" onclick="Pagination_chofer('.($paginaActual+1).');">
    ...
    $consulta_2 = "SELECT id_chofer AS id, expediente_chofer, nombre_chofer, nombre_dep FROM chofer WHERE (expediente_chofer LIKE '%$dato%' OR nombre_chofer LIKE '%$dato%') AND estatus = '1' ORDER BY expediente_chofer DESC LIMIT $limit, $nroLotes";
